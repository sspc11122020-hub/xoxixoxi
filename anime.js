...
// إعدادات المسارات المخصصة للأنمي
const ANIME_DIR = path.join(__dirname, "anime");
const HOME_FILE = path.join(ANIME_DIR, "Home.json");
...
async function fetchPage(url) {
    try {
        console.log(`🌐 [أنمي] جاري جلب: ${url.substring(0, 60)}...`);
        const headers = {
    ...
    try {
        ...
        doc.querySelectorAll("ul.RightTaxContent li, .post-details li, .movie-details li").forEach(li => {
            ...
            if (labelElement) {
                ...
                if (label.includes('قسم') || label.includes('التصنيف')) details["قسم الانمي"] = linkTexts.length > 0 ? linkTexts : [value];
                else if (label.includes('نوع')) details["نوع الانمي"] = linkTexts.length > 0 ? linkTexts : [value];
...
async function startScraping() {
    console.log("🚀 بدء استخراج مسلسلات الأنمي...");
    const url = `${BASE_URL}/category/مسلسلات-انمي/?key=episodes`;
    ...
    doc.querySelectorAll('.Small--Box a.recent--block').forEach((element, i) => {
        ...
        if (itemUrl) {
            ...
            const title = cleanText(element.querySelector('h3.title')?.textContent || `مسلسل أنمي ${i + 1}`);
    ...
    fs.writeFileSync(HOME_FILE, JSON.stringify({
        fileName: "Home.json",
        description: "مسلسلات أنمي الصفحة الأولى حية من الشبكة",
